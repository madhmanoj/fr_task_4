...
export abstract class WasiProcess {
	...
	public async run(imports: string): Promise<number> {
		...
		return new Promise<number>(async (resolve, reject) => {
			...
			this.startMain(wasiService, imports).catch(reject);
			this._state = 'running';
		}).then((exitCode) => {
			this._state = 'exited';
			return exitCode;
	...
	protected abstract startMain(wasiService: WasiService, imports: string): Promise<void>;
